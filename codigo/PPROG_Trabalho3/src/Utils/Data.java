...
public class Data implements Comparable<Data> {
    ...
    public Data (String data) {
        ...
        try{
            ...
            try{
                ano = Integer.parseInt(dataString[0]);
                mes = Integer.parseInt(dataString[1]);
                dia = Integer.parseInt(dataString[2]);
                setData(ano,mes,dia);
            } catch (NumberFormatException j){
                 throw new  DataException_DataInvalida("A Data (" + data + ") não é valida, formato esperado AAAA/MM/DD!");
            }            
        } catch (Exception e){
            throw new  DataException_DataInvalida("A Data (" + data + ") não é valida, formato esperado AAAA/MM/DD!");
        }
    ...
    public final void setData(int ano, int mes, int dia) {
        if (mes < 1 || mes > 12) {
            throw new DataException_MesInvalido("Mês " + mes + " é inválido!!");
        }
        if (dia < 1 || dia > Mes.obterMes(mes).numeroDeDias(ano))  {
            throw new DataException_DiaInvalido("Dia " + ano + "/" + mes + "/" + dia
                    + " é inválido!!");
        }
        this.ano = ano;         
